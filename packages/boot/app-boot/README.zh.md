...
- **源码投影。** ESM 后备请求使用声明依赖的 manifest 的真实路径，并保留已启用的 loader 钩子。因此，[tsx 源码启动器](../../../.agents/notes/implemented/architecture/2026-07-29-dsh-source-launch-tsx-esm.zh.md) 对工作区插件和 CLI 导入应用相同的 `paths` 投影，不依赖 `lib/` 是否存在。普通 Node 仍按包的 exports 解析。
