...
- **Source projection.** ESM fallback requests use the declaring manifest's real path and retain the active loader hooks. The [tsx source launcher](../../../.agents/notes/implemented/architecture/2026-07-29-dsh-source-launch-tsx-esm.md) therefore applies the same `paths` projection to workspace plugins and CLI imports, with or without `lib/` present. Plain Node still resolves package exports.
