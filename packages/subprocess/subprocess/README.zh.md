...
- `scrubbedParentEnv()` / `SENSITIVE_ENV_PATTERN` 是唯一一份共享的环境清理定义：环境中形似凭据的名称、`DSH_*` 名称，以及完整的 indexed `GIT_CONFIG_COUNT`/`KEY_n`/`VALUE_n` 元组都会被丢弃，显式 `env` 在清除之后合并。将 Git 元组作为整体处理，既避免子进程收到残缺配置，也防止凭据形状的 `KEY_n` 被移除后，`VALUE_n` 仍携带环境中的机密。本地的普通 spawn 与终端 spawn 都应用该定义；拥有自身 spawn 的 SDK 管理传输可直接导入它。
