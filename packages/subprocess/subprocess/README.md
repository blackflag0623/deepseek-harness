...
- `scrubbedParentEnv()` / `SENSITIVE_ENV_PATTERN` are the one shared scrub definition: ambient credential-shaped names, `DSH_*` names, and the complete indexed `GIT_CONFIG_COUNT`/`KEY_n`/`VALUE_n` tuple are dropped, and explicit `env` merges after the scrub. Treating the Git tuple atomically avoids malformed child configuration and keeps a `VALUE_n` from carrying an ambient secret after its credential-shaped `KEY_n` was removed. The local ordinary and terminal spawns both apply it; SDK-managed transports that own their spawn may import it directly.
