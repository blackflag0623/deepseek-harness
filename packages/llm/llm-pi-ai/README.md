...
| `maxRequestImages` | unbounded | Retained image-occurrence bound; a request that exceeds it fails with `IMAGE_OFFLOAD_REQUIRED` |
...
- **Request image limits are route-specific** — `maxRequestImageBytes` counts base64 payload only, while `maxRequestImages` counts retained image occurrences including nested tool results. Text, tools, descriptors, and JSON structure remain outside both limits, so configure headroom below the gateway's limits. Exceeding either limit triggers durable oldest-first image offload before retry.
