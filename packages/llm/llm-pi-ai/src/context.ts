...
export interface PiImageRequestContext {
  ...
  /** Request-level image occurrence bound; omission or undefined leaves count unbounded. */
  maxRequestImages?: number | undefined
 ...
 * retained occurrences still exceed `maxRequestImageBytes` or
 * `maxRequestImages`, the call fails with `IMAGE_OFFLOAD_REQUIRED` naming how
 * many more oldest occurrences must be offloaded.
 * @param options - the harness request; `options.system`, else a leading `system` message, maps to pi-ai's single `systemPrompt` slot.
...
async function toPiContextWithImages(
  ...
  const { attachments, resolveImageAccess, maxRequestImageBytes, maxRequestImages } = images
  ...
  if (maxRequestImageBytes !== undefined || maxRequestImages !== undefined) {
    const offloadImages = requiredImageOffload(
      split.messages,
      {
        representation: 'base64',
        ...maxRequestImageBytes === undefined ? {} : { maxBytes: maxRequestImageBytes },
        ...maxRequestImages === undefined ? {} : { maxImages: maxRequestImages },
      },
      block => (requestImages.get(block.attachment.attachmentId) as RequestImageAttachment).bytes,
    )
    if (offloadImages > 0) {
      throw new LlmError(
        `pi-ai request images exceed the route image bounds; ${offloadImages} more oldest occurrence(s) must be offloaded.`,
