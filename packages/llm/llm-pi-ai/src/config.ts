...
export interface ResolvedPiAiProviderProfile
  ...
  maxRequestImageBytes: number
  /** Positive request-level image occurrence bound, or undefined when unbounded. */
  maxRequestImages?: number
...
const profile = z.object({
  ...
  maxRequestImages: z.number().step(1).min(1),
...
export function resolveProfiles(
  ...
  for (const [provider, source] of entries) {
    ...
    const maxRequestImages = source.maxRequestImages
    if (maxRequestImages !== undefined && (!Number.isSafeInteger(maxRequestImages) || maxRequestImages <= 0)) {
    ...
    resolved.set(provider, {
      ...
      ...maxRequestImages === undefined ? {} : { maxRequestImages },
