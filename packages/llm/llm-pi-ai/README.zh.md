...
| `maxRequestImages` | 无上限 | 保留图片出现次数上限，超过时请求以 `IMAGE_OFFLOAD_REQUIRED` 失败 |
...
- **请求图片上限属于具体路由**——`maxRequestImageBytes` 只计算 base64 载荷，`maxRequestImages` 计算保留的图片出现次数，包括嵌套工具结果。文本、工具、描述符与 JSON 结构均不计入这两个上限，因此配置值应留有余量地低于网关上限。超过任一上限都会在重试前触发持久的最旧优先图片省略。
