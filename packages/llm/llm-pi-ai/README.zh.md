...
插件会回答"该提供方可以提供哪些模型？"，供配置界面正在编辑或起草的路由使用。已安装目录提供的路由直接由目录回答，不发网络请求；只有目录未描述的路由才会经网络询问（`openai-completions` 与 `openai-responses` 形状）。多数列表只公布 id；网关提供容量与 `input` 扩展时会保留这些字段。`input` 保留受支持的 `text` 与 `image` 值、丢弃未知值，并成为被采纳模型的模态声明。回答是界面可以提供给用户采纳的候选元数据——采纳之前不存储任何内容，`settings.yaml` 仍然是决定路由服务内容的唯一事实。
