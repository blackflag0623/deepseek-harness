...
Authenticated native clients may instead POST one standard Remote `open` frame to `/api/remote.stream`. The response carries the same `item`, `error`, and `end` frames as newline-delimited JSON; closing the HTTP response aborts that logical stream and awaits its iterator. This single-stream carrier exists for mobile lifecycle and network paths where a long-lived multiplexed WebSocket is unreliable. It does not define another business API: both carriers call the same `wireStream.open()` dispatcher and failure mapper.
