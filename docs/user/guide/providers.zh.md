...
在**模型目录**中选择**获取可用模型**，可查询表单当前显示的基础 URL 和凭据。选择候选项只会更新草稿；保存前不会存储提供方。目录提供方使用已安装目录，不发起网络请求。自定义端点还可以为每个模型返回 `input` 列表；采纳 `input: [text, image]` 元数据后，该模型会在 Harness 中具备图片能力。

### 图片输入

手动输入的模型，或从省略 `input` 的端点发现的模型，一律按纯文本对待。给这类模型附加图片，会在发送前就被拒绝，并点名该模型。
