...
- `LlmDiscoveredModel` makes every field but `id` optional, because most listings disclose an id and nothing else. In addition to names and capacities, its optional `input` carries supported request modalities when the endpoint states them. The reply is candidates, not a catalog: a surface adopting one still owes metadata the adapter requires.
