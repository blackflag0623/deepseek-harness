...
**未声明即 `[text]`，而这是「尚未声明」，不是对端点的猜测。** OpenAI 标准列表不报告模态，但网关可以添加模型发现会保留的 `input` 扩展。当已安装 catalog、settings 与该扩展都没有作答时，唯一诚实的底线是每个受支持协议都确定携带的那个模态。少声明会在图片附加之前拒绝、点名模型，并给出有文档的配置补救方法。多声明会先接纳并持久化图片，再由提供方拒绝。之后对同一错误声明路由的请求还会再次遇到图片，但用户可以选择纯文本模型，因为请求组装会把持久图片投影为占位符。
...
**没有任何配置界面手工编辑 `input`。** 它和 `compat`、`reasoningEfforts`、`thinkingBudgets`、`headers` 一样是 settings 文档字段，而模型列表编辑器仍是一张只覆盖 id、名称和两个容量的手写表单。不过，端点询问可以把 `input` 作为候选元数据提供，采纳时无需新增模态控件即可将其存下。选中的已有行会保留每个用户自有字段，只补齐它未设置的元数据，因此发现可以加入 `input` 而不覆盖已调过的容量；普通行编辑则会先展开已存储的行再应用改动。
...
- **在端点省略时推断模态** —— OpenAI 标准列表不报告模态，因此只有明确的提供方扩展才能作为依据。
- **从模型 id 推断**（`*-vision`、`*-vl`）—— 命名约定不是能力，网关可以随意改名。
- **继续拒绝，只改进诊断** —— 那条消息对状态的描述本来就准确，对补救办法则毫无用处；缺的正是补救办法。

## 影响

自定义提供方下的视觉模型只需在 settings 文档里写一行 `input: [text, image]`——或者当该路由列出的模型全都接受图片时，在路由上写一行。明确返回同一 `input` 元数据的端点，也可以在模型首次被采纳时提供这一行。三个准入点随即接纳它上面的图片，`read_image` 也可用。部署与端点都不声明时，继续保持保守的纯文本行为。
...
`config.spec.ts` 负责 schema 边界：两个层级上的未知模态拒绝、路由空列表被 schema 接受而由 settings seam 真正运行的命名空间校验器拒绝，以及继承规则所倚赖的「缺省数组物化为 `[]`」这一事实。发现测试覆盖了在不新增手工表单控件的情况下采纳端点的 `input` 扩展。
