...
**No configuration surface edits `input` manually.** It joins `compat`, `reasoningEfforts`, `thinkingBudgets`, and `headers` as a settings-document field, and the model-list editor stays a hand-written form over id, name, and the two capacities. Endpoint interrogation may nevertheless supply `input` as candidate metadata, which adoption stores without adding a modality control. A selected existing row keeps every user-owned field and fills only metadata it leaves absent, so discovery can add `input` without replacing a tuned capacity; ordinary row edits spread the stored row before applying changes.
...
- **Infer modalities when the endpoint omits them** — OpenAI's standard listing reports none, so only an explicit provider extension is evidence.
...
A vision model on a custom provider costs one line, `input: [text, image]`, written in the settings document — or one line at the route when every model it lists takes images. An endpoint that explicitly returns the same `input` metadata can supply that line when its model is first adopted. The three admission points then admit images on it and `read_image` works. A deployment and endpoint that state nothing keep the conservative text-only behavior.
...
`config.spec.ts` holds the schema boundary: an unknown modality refused at both levels, the empty route list accepted by the schema and refused by the namespace validator that the settings seam actually runs, and the `[]` materialization for an absent array that the inheritance rule depends on. Discovery tests cover adoption of an endpoint's `input` extension without introducing a manual form control.
