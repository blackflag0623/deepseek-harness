...
describe('CI workflow', () => {
  ...
  it('keeps split native Windows PR jobs with failover, plus a master-only standby', () => {
    ...
    const canonical = 'deepseek-harness/deepseek-harness'
    const evaluate = (
      expression: string,
    ...
    for (const [name, selector, variable, pool, hosted] of [
      ...
      expect(evaluate(selector, { [variable]: 'selfhosted' }, canonical, 'dependabot[bot]'), `${name} dependabot on selfhosted`).toBe(hosted)
      ...
      for (const mode of ['', 'blacksmith', 'selfhosted']) {
        expect(evaluate(selector, { [variable]: mode }, 'owner/deepseek-harness'), `${name} fork on ${mode}`).toBe(
          name === 'windows lanes' ? 'windows-2025' : name === 'linux gates' ? 'ubuntu-24.04' : 'ubuntu-latest',
        )
      }
    }

    expect(nodeCompat['runs-on']).toContain("github.repository == 'deepseek-harness/deepseek-harness'")
